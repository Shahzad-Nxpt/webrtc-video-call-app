...
export default function App({ }) {
  ...
  const [callerId] = useState(
    Math.floor(100 + Math.random() * 90).toString(),
  );
  const otherUserId = useRef(null);

  const socket = SocketIOClient('https://192.168.100.62:3500/', {
  ...
  useEffect(() => {
    ...
    peerConnection.current.onaddstream = event => {
      console.log('peerConnection', event.stream)
  ...
  const WebrtcRoomScreen = () => {
    console.log("callerId", callerId, remoteStream)
    return (
      <View
        style={{
          flex: 1,
